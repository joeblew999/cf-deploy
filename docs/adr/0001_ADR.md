...
3. **Distribution Formats (ADR 0002)** — Distributed as both a **Tiny JS Bundle** (for Bun users via `bun x`) and a **Standalone Binary** (for CI/non-Bun users).
...
```
cf-deploy/
  dist/cf-deploy.js       Tiny JS bundle (for bun x usage)
  cf-deploy               Compiled standalone binary (for zero-bun usage)
